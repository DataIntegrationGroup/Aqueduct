...
| 2026-09-16 | BernCo HydroVu validated end to end against local docker FROST (ST2DAT-132). `initial_start_date` moved to `2026-09-09`, a 7-day window, which moves the sentinel floor with it. Two runs of `bernco_hydrovu_pipeline`: the first loaded one Thing, Location and Datastream (`SierraVista-966932`; `WhisperingPines-1002958` has no readings since the start date) with 12 observations. The second re-fetched the boundary reading, because HydroVu's `startTime` is inclusive, and the FROST watermark skipped it, so nothing was posted twice. `tests/defs/test_definitions.py` now checks that every registry source's transform result carries the `.bundles` / `.max_load_id` fields the load asset reads. |
