...
@patch("aqueduct_dagster.sources.bernco_hydrovu.backfill.load_source_config")
...
def test_default_backfill_location_ids_is_empty_when_key_not_configured(mock_cfg):
    mock_cfg.return_value = {"gcp_secret": "hydrovu_bernco"}  # no location_ids key
...
@patch("aqueduct_dagster.sources.bernco_hydrovu.backfill.fetch_locations")
@patch("aqueduct_dagster.sources.bernco_hydrovu.backfill.build_hydrovu_client")
@patch("aqueduct_dagster.sources.bernco_hydrovu.backfill.load_source_config")
...
@patch("aqueduct_dagster.sources.bernco_hydrovu.backfill.read_parquet_rows_for_load_id")
@patch("aqueduct_dagster.sources.bernco_hydrovu.backfill.run_backfill_ingest")
...
def test_run_backfill_chunk_reports_adapter_failures_without_dropping_good_locations(
    ...
    """One location's reading has a malformed timestamp — BerncoHydroVuAdapter
    raises adapting it, BaseAdapter.run() catches and records it. The other,
    healthy location must still produce a bundle and get loaded."""
    ...
    assert result.bundles_loaded == 1  # only location 111's bundle survives
...
@patch("aqueduct_dagster.sources.bernco_hydrovu.backfill.read_parquet_rows_for_load_id")
@patch("aqueduct_dagster.sources.bernco_hydrovu.backfill.run_backfill_ingest")
def test_run_backfill_chunk_applies_the_sentinel_floor(mock_run_ingest, mock_read_rows):
    """A near-epoch bad-clock reading must be dropped; real historical data,
    e.g. from 2009, must not be — this is the exact bug a shared-with-production
    floor would cause, since 2009 is also "before initial_start_date"."""
    mock_run_ingest.return_value = 100.0
    mock_read_rows.return_value = (
        [
            {  # bad device clock — must be dropped
                "location_id": 111,
                "parameter_id": "4",
                "unit_id": "35",
                "timestamp": 960,
            ...
            {  # real 2009 history — must survive
                "location_id": 222,
                "parameter_id": "4",
                "unit_id": "35",
                "timestamp": 1242864000,  # 2009-05-21
                "value": 10.0,
            },
        ],
        frozenset(),
    ...
    assert result.observations_posted == 1  # only location 222's 2009 reading survives
    all_records = [r for call in loader.load_window_calls for r in call[2]]
    assert len(all_records) == 1
    assert all_records[0].result == pytest.approx(10.0 * 3.28084)
...
def test_run_backfill_chunk_handles_empty_loads_ids_without_crashing(
    ...
    """run_backfill_ingest returns None when the chunk yields zero rows — must
    return a zero ChunkResult, not crash."""
