...
class TestToThing:
    def test_external_key_uses_integer_location_id(self):
        ...
        thing = PvacdHydroVuAdapter([rec]).to_thing(rec)
...
class TestToObservations:
    def test_returns_only_dtw_readings(self):
        ...
        obs = PvacdHydroVuAdapter([rec]).to_observations(rec)
        assert len(obs) == 1

    def test_skips_temperature(self):
        rec = _record(readings=[TEMP_READING])
        assert PvacdHydroVuAdapter([rec]).to_observations(rec) == []
...
class TestBuildDatastreams:
    def _make_thing(self):
        rec = _record()
        adapter = PvacdHydroVuAdapter([rec])
...
class TestRun:
    def test_one_bundle_per_location(self):
        ...
        bundles = list(PvacdHydroVuAdapter(records).run())
    ...
    def test_bundle_observations_keyed_by_datastream(self):
        ...
        bundles = list(PvacdHydroVuAdapter([rec]).run())
    ...
    def test_empty_records_yields_no_bundles(self):
        assert list(PvacdHydroVuAdapter([]).run()) == []
    ...
    def test_bad_record_is_skipped_and_counted_as_failure(self):
        ...
        adapter = PvacdHydroVuAdapter([bad_record])
